...
class Seat:
    ...
    def getSeatInfo(self):
        ...
        start = datetime(self.date.year, self.date.month, self.date.day, 8, 0)
    ...
    def getMap(self):
        mapId = {self.dfId['id'][i][-4:]: (self.dfId['seatNumber'][i], self.dfId['area'][i]) for i in range(self.dfId.shape[0])}
